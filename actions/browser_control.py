...
An independent _BrowserSession instance is maintained for each browser. The active browser can be changed with the "switch" action.

NOTE: All browsers open with the user's real profile (accounts and cookies are preserved).
...
# ── Real user profile directory (all browsers) ─────────────────────────
...
class _BrowserSession:
    """
    A full session for a browser instance.
    All browsers open on the actual profile with launch_persistent_context.
    """
    ...
    async def _launch(self):
        """
        It starts the browser with the actual user profile.
        If Context is already open, it does nothing.
        """
...
class _SessionRegistry:
    """It manages all active browser sessions."""
...
def browser_control(
    ...
    Parameters:
        browser : chrome | edge | firefox | opera | operagx | brave | vivaldi | safari
                  (Unless otherwise specified, the active browser will be used.)
    """

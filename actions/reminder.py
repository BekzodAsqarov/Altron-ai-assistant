...
try:
    ...
    notification.notify(title="ALTRON Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("ALTRON Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "ALTRON Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by ALTRON reminder — do not edit
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.altron.reminder.{task_name}"
...
def reminder(
    ...
    task_name  = f"AltronReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"

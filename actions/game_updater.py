...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not colorful.any():
            print("[GameUpdater] ⚠️ Avatar color not found — click based on guesswork.")
        ...
        print(f"[GameUpdater] 🎯 Profile avatar ({abs_x}, {abs_y}) — being clicked")
...
def _handle_steam_profile_selection() -> bool:
    print("[GameUpdater] 🔍 Profile selection dialog is being checked...")
    win = _get_steam_window_rect()
    if not win:
        return False
    ...
    print("[GameUpdater] 👤 Profile selection detected — clicking on the first profile.")
        ...
        try:
            for ctrl in dialog.descendants(control_type=control_type):
                if target in ctrl.window_text().upper():
                    ctrl.click_input()
                    print(f"[GameUpdater] ✅ Driver selected ({control_type}): {ctrl.window_text()}")
                    return True
        except Exception:
            continue
    try:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    print(f"[GameUpdater] 🏆 Target driver: {drive_label} ({best_drive['free_gb']:.1f} GB empty)")
    ...
    try:
        ...
        for _ in range(40):
            ...
            try:
                for hwnd in findwindows.find_windows(
                    title_re=r"(?i)(install|upload|steam)", visible_only=True
                ):
                    try:
        ...
        install_clicked = _click_button(
            dialog, ["install", "upload", "next", "forward", "ok", "tamam"]
        )
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    print("[GameUpdater] 🚀 Steam is starting...")
    if is_mac():
    ...
    for _ in range(20):
        time.sleep(1)
        if _is_steam_running():
            print("[GameUpdater] ✅ Steam is working.")
            time.sleep(4)
            if is_windows():
...
def _install_steam_game(steam_path: Path, game_name: str = None,
    ...
    if not app_id and game_name:
        ...
        print(f"[GameUpdater] 🔍 Established: {game_name} (AppID: {app_id})")
